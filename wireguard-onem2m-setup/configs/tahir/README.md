...
- `[TAHIR_LAN_SUBNET]` → your home LAN subnet (e.g. `192.168.3.0/24`)
- `[ALPER_LAN_SUBNET]`, `[BENJAMIN_LAN_SUBNET]` → other homes' LAN subnets if/when provided
...
The config includes a PostUp rule that adds a MASQUERADE rule for traffic from your home LAN to the VPN. Confirm the source subnet you set in `[TAHIR_LAN_SUBNET]` is correct. If your LAN interface is not `eth0` (e.g., `wlan0`), update the PostUp/PostDown commands to use the correct interface name.
...
sudo iptables -t nat -D POSTROUTING -s [TAHIR_LAN_SUBNET] -o wg0 -j MASQUERADE
