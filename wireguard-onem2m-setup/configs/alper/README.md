...
- `[ALPER_LAN_SUBNET]` → your home LAN subnet (e.g. `192.168.1.0/24`)
- `[BENJAMIN_LAN_SUBNET]`, `[TAHIR_LAN_SUBNET]` → other homes' LAN subnets if/when provided
...
The config includes a PostUp rule that adds a MASQUERADE rule for traffic from your home LAN to the VPN. Confirm the source subnet you set in `[ALPER_LAN_SUBNET]` is correct. If your LAN interface is not `eth0` (e.g., `wlan0`), update the PostUp/PostDown commands to use the correct interface name.
...
sudo iptables -t nat -D POSTROUTING -s [ALPER_LAN_SUBNET] -o wg0 -j MASQUERADE
